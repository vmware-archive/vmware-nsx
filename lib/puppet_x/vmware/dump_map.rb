...
require 'puppet_x/vmware/mapper_nsx'
...
map = PuppetX::VMware::MapperNsx.new_map(map_type)

max_length_prop_name = map.leaf_list.
    map{|element| element.prop_name}.
    reduce(max = 0) {|max, element| puts max; puts element ; sz = element.length ; max = sz if sz > max ; max }

spaces = "\t"
spaces = ' ' * (1 + max_length_prop_name)

map.leaf_list.each do |leaf|
  path = leaf.path_should
  # puts "#{leaf.prop_name}\t=> nested_value($spec, #{path.inspect}),"
  padding = max_length_prop_name - leaf.prop_name.length + 1
  spaces = ' ' * padding
  s = '[' << path[1..-1].reduce("'#{path[0]}'"){|r,e| "#{r}, '#{e}'"} << ']'
  puts "#{leaf.prop_name}#{spaces}=> nested_value($spec, #{s}),"
end 
