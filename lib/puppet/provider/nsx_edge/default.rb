...
Puppet::Type.type(:nsx_edge).provide(:nsx_edge, :parent => Puppet::Provider::Nsx) do
  @doc = 'Manages NSX edge.'
  ...
  { :enable_aesni     => 'aesni?enable=',
    ...
    define_method("#{property}=".to_sym) do |value|
      post("api/4.0/edges/#{@instance['id']}/#{request}#{value}", {})
    end
  end
  ...
  def process_vnic
    ...
    resource[:vnics].each_with_index do |vnic,index|
      vnic['portgroupId'] = portgroup_moref(vnic['portgroupName'], vnic['isLogical'])
      vnic['index']     ||= index
      all_vnics << vnic.reject{ |x| x == 'isLogical' }
    end
    all_vnics
  end
  ...
  def create
    ...
    data = {
      ...
      :description => 'NSX Edge managed by Puppet',
    ...
    @edge_summary = nil 
    post("api/4.0/edges",:edge => data)
  end

  def destroy
    delete("api/4.0/edges/#{@instance['id']}")
  end

  def vnics
    vnic_url = "api/4.0/edges/#{@instance['id']}/vnics"
  ...
  def vnics=(nics)
    resource[:vnics].each do |new_vnic|
      ...
      if cur_vnic.nil?
        new_vnic['portgroupId'] = portgroup_moref(new_vnic['portgroupName'], new_vnic['isLogical'])
        new_vnic['index']     ||= next_avail_vnic['index']
        vnic_url                = "/api/4.0/edges/#{@instance['id']}/vnics/?action=patch"

        Puppet.debug("Adding vnic#{new_vnic['index']}")
        post("#{vnic_url}", {:vnics => {:vnic => new_vnic.reject{ |x| x == 'isLogical' } } } )
      else
        data        = {}
        data[:vnic] = cur_vnic.merge(new_vnic.reject{ |x| x == 'isLogical' })
        vnic_url    = "/api/4.0/edges/#{@instance['id']}/vnics/#{cur_vnic['index']}"
  ...
  def cli_settings
    nested_value(get("/api/4.0/edges/#{@instance['id']}"), ['edge','cliSettings'])
  end

  def cli_settings=(value)
    put("/api/4.0/edges/#{@instance['id']}/clisettings", :cliSettings => value )
  end
