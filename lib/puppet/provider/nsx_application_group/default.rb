...
Puppet::Type.type(:nsx_application_group).provide(:default, :parent => Puppet::Provider::Nsx) do
  @doc = 'Manages NSX Application Group, application groups ( service groups ) can consist of one or more applications and/or other application groups'

  def exists?
    results = ensure_array( nested_value(get("/api/2.0/services/applicationgroup/scope/#{nsx_scope_moref}"), ['list', 'applicationGroup']) )
  ...
  def create
    ...
    post("api/2.0/services/applicationgroup/#{nsx_scope_moref}", {:applicationGroup => data} )
  ...
  def add_app_members(app_group,app_grp_id)
    ...
    app_url = "/api/2.0/services/application/scope/#{nsx_scope_moref}"
  ...
  def add_app_grp_members(app_group,app_grp_id)
    app_grp_url = "/api/2.0/services/applicationgroup/scope/#{nsx_scope_moref}"
