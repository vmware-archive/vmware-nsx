...
Puppet::Type.type(:nsx_vxlan_multicast).provide(:default, :parent => Puppet::Provider::Nsx) do
  ...
  def replace_properties
    data = {}
    Puppet::Type.type(:nsx_vxlan_multicast).parameters.collect.reject{|x| x.to_s =~ /(scope_name|provider)/}.each do |prop|
      if resource[prop]
