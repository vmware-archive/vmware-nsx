...
Puppet::Type.type(:nsx_ipset).provide(:default, :parent => Puppet::Provider::Nsx) do
  @doc = 'Manages NSX ipset.'

  def exists?
    results = ensure_array( nested_value(get("/api/2.0/services/ipset/scope/#{nsx_scope_moref(resource[:scope_type])}"), ['list', 'ipset']) )
  ...
  def create
    ...
    post("api/2.0/services/ipset/#{nsx_scope_moref(resource[:scope_type])}", {:ipset => data} )
  end
