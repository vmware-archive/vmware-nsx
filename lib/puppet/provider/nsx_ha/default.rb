...
Puppet::Type.type(:nsx_ha).provide(:default, :parent => Puppet::Provider::Nsx) do
  @doc = 'Manages NSX ha service.'

  def edge_ha
    @edge_ha ||= begin
      ha_url  = "/api/4.0/edges/#{nsx_edge_moref}/highavailability/config"
  ...
  Puppet::Type.type(:nsx_ha).properties.collect{|x| x.name}.reject{|x| x == :ensure or x == :ip_addresses}.each do |prop|
  ...
  def validate_vnic(vnic)
    vnic_url   = "api/4.0/edges/#{nsx_edge_moref}/vnics"
  ...
  def appliances
    appl_url = "/api/4.0/edges/#{nsx_edge_moref}/appliances" 
  ...
  def flush
    if @pending_changes
      ...
      put("api/4.0/edges/#{nsx_edge_moref}/highavailability/config", data )
    end
    if @appliance_changes
      appliances.each_with_index do |cur_appl,index|
        ...
        if cur_appl != new_appl
          ...
          appl_url = "/api/4.0/edges/#{nsx_edge_moref}/appliances/#{appl_ha_index}"
