...
Puppet::Type.type(:nsx_vxlan_segment).provide(:default, :parent => Puppet::Provider::Nsx) do
  ...
  def vxlan_segment
    @vxlan_segment ||= begin
      ...
      results = ensure_array( nested_value(get(segment_url), %w{segmentRanges segmentRange}))
      name = resource['name']
  ...
  def replace_properties
    data = {}
    Puppet::Type.type(:nsx_vxlan_segment).parameters.collect.reject{|x| x.to_s =~ /(scope_name|provider)/}.each do |prop|
      if resource[prop]
