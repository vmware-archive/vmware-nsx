...
Puppet::Type.type(:nsx_vxlan_switch).provide(:default, :parent => Puppet::Provider::Nsx) do
  ...
  def replace_properties
    data = {}
    Puppet::Type.type(:nsx_vxlan_switch).parameters.collect.reject{|x| x.to_s =~ /(name|path|scope_name|provider)/}.each do |prop|
      if resource[prop]
