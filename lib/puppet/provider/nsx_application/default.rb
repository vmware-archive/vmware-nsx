...
Puppet::Type.type(:nsx_application).provide(:default, :parent => Puppet::Provider::Nsx) do
  @doc = 'Manages NSX application.'

  def exists?
    results = ensure_array( nested_value(get("/api/2.0/services/application/scope/#{nsx_scope_moref}"), ['list', 'application']) )
  ...
  def create
    ...
    post("api/2.0/services/application/#{nsx_scope_moref}", {:application => data} )
  end
