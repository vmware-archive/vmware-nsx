...
Puppet::Type.type(:nsx_nat).provide(:default, :parent => Puppet::Provider::Nsx) do
  @doc = 'Manages NSX nat rules '

  def edge_nat
    @edge_nat ||= begin
      nat_url   = "/api/4.0/edges/#{nsx_edge_moref}/nat/config"
  ...
  Puppet::Type.type(:nsx_nat).properties.collect{|x| x.name}.reject{|x| x == :ensure}.each do |prop|
  ...
  def create
    post_url = "api/4.0/edges/#{nsx_edge_moref}/nat/config/rules"
    post("#{post_url}", { 'natRules' => { 'natRule' => replace_properties } } )
  end

  def destroy
    delete("api/4.0/edges/#{nsx_edge_moref}/nat/config/rules/#{edge_nat['ruleId']}")
  end
  ...
  def flush
    if @pending_changes
      ...
      put_url = "api/4.0/edges/#{nsx_edge_moref}/nat/config/rules/#{edge_nat['ruleId']}"
