...
Puppet::Type.type(:nsx_dhcp).provide(:default, :parent => Puppet::Provider::Nsx) do
  @doc = 'manage nsx edge dhcp service.'
  
  def edge_dhcp
    @edge_dhcp ||= begin     
      results = nested_value(get("/api/4.0/edges/#{nsx_edge_moref}/dhcp/config"), [ 'dhcp' ] )
  ...
  def flush
    if @pending_changes
      ...
      put("api/4.0/edges/#{nsx_edge_moref}/dhcp/config", { 'dhcp' => edge_dhcp } ) 
    end
  end
end
