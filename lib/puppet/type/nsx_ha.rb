...
Puppet::Type.newtype(:nsx_ha) do
  @doc = 'Manage NSX ha settings'
  ...
  newparam(:scope_name, :namevar => true) do
    desc 'name of the nsx edge to enable ha on'
  end
  ...
  newproperty(:declare_dead_time) do
    desc 'this is value in which nsx will consider the ha node dead if not responsive'
    newvalues(/^\d+$/)
    defaultto(6)
  end
